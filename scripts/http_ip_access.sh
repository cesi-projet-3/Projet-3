...
filename="/root/projet/access_$date$time.csv" #We name the .csv file with date and time
...
cat /var/log/apache2/access.log | sed -r -e 's/(([0-9]+\.){3}[0-9]+) .+(([0-9]{2}:){2}[0-9]{2}).+/\3,\1/' >> $filename
